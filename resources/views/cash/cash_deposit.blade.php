        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <form method="POST" action="{{ route('post.cash.deposit') }}">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group row">
                            <div class="col-md-6">
                                <select class="form-control" name="bank" required>
                                    @if(count($banks) > 0)
                                        @foreach($banks as $bank)
                                            <option @if( old('bank') == $bank->id ) @endif value="{{ $bank->id }}">{{ $bank->name }} ({{ $bank->branch }})</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
        ...
        $(document).ready(function () {
            ...
            function validateBillNo(bill_no = undefined, userId = undefined) {
                ...
                if(bill_no && userId){
                    $.ajax({
                        type: 'post',
                        url: "{{ route('api.validate.cash.deposit.voucherno') }}",
                        data: {
                            "token": bill_no,
                            "user": userId
                        },
