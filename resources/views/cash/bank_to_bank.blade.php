        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <form method="POST" action="{{ route('post.bank.to.bank.transfer') }}">
                        ...
                        <div class="form-group row">
                                ...
                                <select class="form-control" name="from_bank">
                                    @if(count($banks) > 0)
                                        ...
                                        @foreach($banks as $bank)
                                            <option @if( old('from_bank') == $bank->id ) selected="selected" @endif value="{{ $bank->id }}">{{ $bank->name }} ({{ $bank->branch }})</option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="to_bank">
                                    @if(count($banks) > 0)
                                        <option>Select Bank</option>
                                        @foreach($banks as $bank)
                                            <option @if( old('to_bank') == $bank->id ) selected="selected" @endif value="{{ $bank->id }}">{{ $bank->name }} ({{ $bank->branch }})</option>
                                        @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="col-md-4">
                                {{-- <label>Voucher</label>
                                <input type="text" class="form-control" name="voucher_no" value="{{ old('voucher_no') }}" />
                                ...
                                <label>Voucher</label>
                                <div class="form-group {{ $showErrors ? ' has-error' : '' }}">
                                    <input id="voucher_no" type="text" class="form-control" placeholder="Contra No" name="voucher_no" @if ( $myerrors->has('contra') ) required @else @if($errors->has('contra')) {{-- readonly value="{{ $invoice_no + 1 }}" --}} @else @if(isset(auth()->user()->cashSetting) && auth()->user()->cashSetting->bill_no_type == 'auto') value="{{ $contra + 1 }}" readonly @endif @endif @endif required />
                                    @if ($myerrors->has('contra'))
                                        <span class="help-block">
                                            <ul>
                                                @foreach( $myerrors['contra'] as $error )
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="date" type="text" class="form-control custom_date" placeholder="DD/MM/YYYY" name="date" value="{{ old('date') }}" />
                                @if ($errors->has('date'))
                                    <span class="help-block">
...
@section('scripts')
    <script>
        $(document).ready(function () {
            ...
            $('select[name="from_bank"]').on("change", function() {
                let val = $(this).find(":selected").val();

                console.log(val);

                // $('input[name="to_bank"] option[value="'+val+'"]').attr('disabled', true);
                $('select[name="to_bank"] option').filter(function() { 
