                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th colspan="{{ auth()->user()->profile->add_lump_sump == "no" ? 8 : 4 }}" class="text-center">
                                ...
                                <tr>
                                    ...
                                    <th colspan="2" class="text-center inwards_side">Inwards</th>
                                    <th colspan="2" class="text-center outwards_side">Outwards</th>
                                    @endif
                                    <th @if(auth()->user()->profile->add_lump_sump == "no") colspan="2" @endif class="text-center closing_balance_side">Closing Balance</th>
                                </tr>
                                <tr>
                                    @if(auth()->user()->profile->add_lump_sump == "no")
                            ...
                            <tbody>
                                ...
                                <tr>
                                        ...
                                        <select class="form-control rate-select">
                                            @php
                                                ...
                                                if( $physical_stock > 0 ) {
                                                    ...
                                                    $physical_stock_base = $physical_stock;
                                                        
                                                    if($item->has_alternate_unit == "yes"){
                                                        $physical_stock_alternate = ($item->conversion_of_alternate_to_base_unit_value == 0) ? 0 : $physical_stock / $item->conversion_of_alternate_to_base_unit_value;
                                                    }

                                                    if($item->has_compound_unit == "yes") {
                                                        $physical_stock_compound = ($item->conversion_of_compound_to_alternate_unit_value == 0 || $item->conversion_of_alternate_to_base_unit_value == 0) ? 0 : $physical_stock / ($item->conversion_of_compound_to_alternate_unit_value * $item->conversion_of_alternate_to_base_unit_value);
                                                    }
                                    ...
                                    @php 
                                        ...
                                        $purchase_value = auth()->user()->profile->add_lump_sump == "no" ?  $purchase_qty * $purchase_rate : $purchase_rate;

                                        $sale_qty = $item->soldQty ?? 0;
                                        $sale_rate = $item->soldRate ?? 0;
                                        $sale_value = auth()->user()->profile->add_lump_sump == "no" ? $sale_qty * $sale_rate : $sale_rate;

                                        $closing_qty = $opening_stock + $purchase_qty - $sale_qty;
                                        $closing_value = auth()->user()->profile->add_lump_sump == "no" ? $opening_value + $purchase_value - $sale_value : $opening_value + $purchase_value + (auth()->user()->profile->gp_percent_on_sale_value * $sale_value / 100) - $sale_value;
                                    @endphp
                                    ...
                                    <td><a href="{{ route('single.item.report', $item->id) }}">View Detail</a></td>
                                    @endif
                                </tr>
                                @endforeach
                                @else
