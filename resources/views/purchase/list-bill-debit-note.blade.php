    ...
    <div class="table-responsive">
        <table class="table table-bordered">
            ...
            <tbody id="dynamic-body">
                ...
                @php $count = 1; @endphp
                @foreach($debit_notes as $record)
                <tr>
                    <td>{{ $count++ }}</td>
                    @php $note_no = $record->note_no ?? 0 @endphp
                    <td>
                        {{-- @if($record->status) --}}
                            <a href="{{ route('show.purchase.debit.note', $note_no) }}">{{ $note_no }}</a>
                        {{-- @else
                            {{ $note_no }}
                        @endif --}}
                    </td>
                    ...
                    <td>
                        <form method="POST" action="{{ route('update.debit.note.status', $note_no) }}">
                            {{ csrf_field() }}
                            @if(!$record->status)
                    ...
                    <td><a href="{{ route('print.purchase.debit.note', $note_no) }}" class="btn btn-primary">Print</a></td>
                </tr>
                @endforeach
                @else
                    <tr>
