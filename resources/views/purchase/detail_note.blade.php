...
{!! Breadcrumbs::render('purchase-detail-debit-note', request()->segment(2)) !!}
    ...
    <form id="create_note_form" method="post" action="{{ route('bill.create.debit.note', $purchase->id) }}">
        {{ csrf_field() }}
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th colspan="2">
                                @if(auth()->user()->profile->inventory_type != "without_inventory") Revised Qty @endif
                            </th>
                            {{-- <th>Diff Qty</th> --}}
                    ...
                    <tbody>
                        ...
                        @if(count($purchase->purchase_items) > 0)
                            ...
                            <tr>
                                ...
                                <td><input data-gst="{{ $purchase->party->registered == 0 || $purchase->party->registered == 3 ? 0 : $purchase_item->item->gst ?? 0 }}" data-inventory_type="{{ auth()->user()->profile->inventory_type }}" type="text" class="form-control" name="price_difference[]" value="{{ $purchase_item->price ?? 0 }}" /></td>
                                ...
                                <td>
                                    <input type="text" class="form-control" name="gst_difference[]" value="{{ $purchase->party->registered == 0 || $purchase->party->registered == 3 ? 0 : $purchase_item->gst ?? 0 }}" @if($purchase->party->registered == 0 || $purchase->party->registered == 3) readonly @endif />
                                </td>
                                ...
                                <td><input data-gst="{{ $purchase->party->registered == 0 || $purchase->party->registered == 3 ? 0 : $purchase_item->item->gst ?? 0 }}" type="text" class="form-control" name="quantity_difference[]" value="{{ auth()->user()->profile->inventory_type == "without_inventory" ? 0 : $purchase_item->qty ?? 0 }}" @if(auth()->user()->profile->inventory_type == "without_inventory") readonly style="visibility:hidden" @endif /></td>
                                <td>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('input[name="note_no"]').on("keyup", function() {
                ...
                $.ajax({
                    method: 'POST',
                    url: "{{ route('api.purchase.debitnote.validate.noteno') }}",
                    data: {
                        note_no: note_no
                    },
            ...
            $('#note_reason_change').on("change", function() {
                ...
                if( $(this).val() == "discount_on_purchase" ){
                    $('input[name="price_difference[]"]').attr("readonly", false);
                    ...
                    $('input[name="price_difference[]"]').attr("readonly", true);
