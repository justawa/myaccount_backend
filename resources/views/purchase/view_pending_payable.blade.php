...
<div class="container" id="document">
    ...
    <div class="row">
        <div class="col-md-12">
                        ...
                        <div class="col-md-4">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu" role="menu" style="padding: 5px 10px;">
                                    <form id="search_pending_payable_form">
                                    <li>
                                        <div class="form-group">
            ...
            <li class="active"><a data-toggle="tab" href="#bill">Bill Wise</a></li>
            ...
            <div id="bill" class="tab-pane fade in active">
                <table class="table table-bordered" id="document_data_table">
                    ...
                    <tbody id="dynamic-body-bill">
                        {{-- @if( count($purchase_amounts) > 0 )
                            @php $count = 1; @endphp
                            @foreach( $purchase_amounts as $amount )
                            <tr>
                                <td>{{ $count++ }}</td>
                                <td><a href="{{ route('view.party.pending.payable.detail', $amount->id) }}">{{ $amount->voucher_no }}</a></td>
                                <td>{{ $amount->payment_date }}</td>
                                <td>{{ $amount->amount_paid }}</td>
                                <td>
                                    <form method="POST" action="{{ route('update.purchase.pending.payable.status', $amount->id) }}">
                                        {{ csrf_field() }}
                        ...
                        @else
                            <tr>
                                <td colspan="5"><span class="text-center">No Payables</span></td>
                            </tr>
                        @endif --}}
                    </tbody>
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            $("#search_pending_payable_form").on("submit", function(e) {
                e.preventDefault();
                get_payable();
            });
                        ...
                        success: function(response){
                            ...
                            if (response.bill.length > 0) {
                                ...
                                for(var i = 0; i < response.bill.length; i++){
                                    ...
                                    let voucher_url = response.bill[i].voucher_url ? response.bill[i].voucher_url : `edit/purchase-pending-payment/${response.bill[i].id}`;
                                    $("#dynamic-body-bill").append(`
                                        <tr>
                                            ...
                                            <td>
                                                <form method="POST" class="status_update_form" action="update-purchase-pending-payable-status/${response.bill[i].id}">
                                                    ${form_csrf}
                            ...
                            if (response.account.length > 0) {
                                ...
                                for(var i = 0; i < response.account.length; i++){
                                    ...
                                    $("#dynamic-body-account").append(`
                                        <tr>
                                            <td>${i+1}</td>
                                            <td><a target="_blank" href="edit/purchase-party-pending-payment/${response.account[i].id}">${response.account[i].voucher_no}</a></td>
                                            <td>${response.account[i].payment_date || ''}</td>
                                            <td>${response.account[i].amount}</td>
                                            <td>
                                                <form method="POST" class="status_update_form" action="update-party-purchase-pending-payable-status/${response.account[i].id}">
                                                    ${form_csrf}
