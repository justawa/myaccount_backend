...
@section('content')
{!! Breadcrumbs::render('find-invoices-by-party') !!}
<div class="container">
    @if (session('success'))
    ...
    @endif
    <form action="{{ route('post.find.invoice.by.party') }}">
            ...
            <div class="panel panel-default">
                ...
                <div style="padding-top: 15px; display: flex">
                    ...
                    <div class="col-md-6 text-right">
                        <button style="display: none;" class="btn btn-success" id="btn_payment_against_party" data-party="">Receipt on Account</button>
                    </div>
                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p><strong>Balance: <span id="total_pending_payment_amount_in_modal"></span></strong></p>
                <form id="form-note" method="post" action="{{ route('add.pending.payment.to.party.sale') }}">
                    {{ csrf_field() }}
                    ...
                    <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <input type="checkbox" name="type_of_payment[]" value="discount" id="discount" /> <label for="discount">Cash Discount</label>
                            </div>
                    ...
                    <div class="form-group {{ $showErrors ? ' has-error' : '' }}">
                        <label>Voucher No.</label>
                        <input type="text" id="voucher_no" class="form-control" name="voucher_no" placeholder="Voucher No." @if ( $myerrors->has('voucher_no') ) required @else @if($errors->has('voucher_no')) {{-- readonly value="{{ $invoice_no + 1 }}" --}} @else @if(isset(auth()->user()->receiptSetting) && auth()->user()->receiptSetting->bill_no_type == 'auto') value="{{ $voucher_no + 1 }}" readonly @endif @endif @endif />
        ...
        $(document).ready(function() {
            ...
            function validateBillNo(party = null, bill_no = null, userId = null) {
                ...
                if(party && bill_no && userId){
                    $.ajax({
                        type: 'post',
                        url: "{{ route('api.validate.sale.party.payment.voucherno') }}",
                        data: {
                            "token": bill_no,
                            "party": party,
                            "user": userId
            ...
            function getPartyData(selected_one, from_date = null, to_date = null) {
                if (selected_one > 0) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "{{ route('post.find.invoice.by.party') }}",
                        data: {
                        ...
                        success: function(response){
                            ...
                            if (response.sale.length > 0) {
                                for(var i = 0; i < response.sale.length; i++){
                                    if (response.sale[i].remaining_amount == null) {
                                        html_row = 'NA';
                                    } else {
                                        html_row = response.sale[i].remaining_amount.amount_remaining;
                                    }
                                    $("#dynamic-body").append(`
                                        <tr>
                                            <td><a href="sale/edit/invoice/${response.sale[i].id}">${response.sale[i].invoice_no}</a></td>
                                            <td>${response.sale[i].invoice_date}</td>
                                            <td>${response.sale[i].due_date}</td>
                                            <td>${response.sale[i].total_amount}</td>
                                            <td>${html_row}</td>
                                            <td><a href="get-invoice-info/${response.sale[i].id}/party/${response.sale[i].party_id}">Add</a></td>
                                        </tr>
                                    `);
