...
{!! Breadcrumbs::render('invoice-detail-credit-note', request()->segment(2)) !!}
    ...
    <form id="create_note_form" method="post" action="{{ route('invoice.create.credit.note', $invoice->id) }}">
        {{ csrf_field() }}
        ...
        <div class="container">
            ...
            <div class="row" style="margin-bottom: 20px;">
                ...
                <div class="col-md-6"><strong>Invoice no</strong> : @if($invoice->invoice_no != null) {{ $invoice->invoice_no }} @else {{ $invoice->id }} @endif</div>
            </div>
            <div class="row" style="margin-bottom: 20px;">
                <div class="col-md-6">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        @if(count($invoice->invoice_items) > 0)
                            ...
                            <tr>
                                <td>{{ $count++ }}</td>
                                <td>{{ $invoice_item->item->name }}<input type="hidden" name="item_id[]" value="{{ $invoice_item->item->id }}" /></td>
                                ...
                                <td><input data-gst="{{ auth()->user()->profile->registered == 0 || auth()->user()->profile->registered == 3 ? 0 : $invoice_item->item->gst ?? 0 }}" type="text" class="form-control" name="quantity_difference[]" value="{{ auth()->user()->profile->inventory_type == "without_inventory" ? 0 : $invoice_item->item_qty ?? 0 }}" @if(auth()->user()->profile->inventory_type == "without_inventory") readonly style="visibility:hidden" @endif /></td>
                                <td>
                        ...
                        <tr>
                            ...
                            <th colspan="3">
                                <select class="form-control" name="reason_change" id="note_reason_change" required>
                                    ...
                                    <option value="discount_on_sale">Discount</option>
                                </select>
                            </th>
                        </tr>
                        <tr>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('input[name="note_no"]').on("keyup", function() {
                ...
                $.ajax({
                    method: 'POST',
                    url: "{{ route('api.sale.creditnote.validate.noteno') }}",
                    data: {
                        note_no: note_no
                    },
            ...
            {
                var qty = $("#quantity_difference").val() || 0;
                var gst = $("#item_gst").val() || 0;
                var price = $("#price_difference").val() || 0;
                var discount = $("#discount_difference").val() || 0;
