...
{!! Breadcrumbs::render('get-invoice-info', request()->segment(2)) !!}

<div class="container">
    <div class="row">
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading">View Invoice Amounts</div>
                <div class="panel-body">
                    <div style="padding: 10px 0; width: 100%; display: inline-block;">
                        ...
                        <div class="col-md-3">
                            <button type="button" id="add-pending-payment" class="btn btn-success">Receipt Bill wise</button>
                        </div>
                    </div>
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            @php $total_paid += $sale->amount_paid; $amount_remaining = $sale->amount_remaining; @endphp
                            @if ($loop->first) @continue @endif
                            <tr>
                                {{-- <td>{{ $count++ }}</td> --}}
                                <td>{{ $sale->type_of_payment }} @if($sale->type_of_payment == 'bank') ({{ $sale->bank_name . " - " . $sale->bank_branch }}) @endif</td>
                                <td>{{ $sale->amount_paid }}</td>
                                <td>{{ $sale->amount_remaining }}</td>
                                <td><a href="{{ route('edit.sale.pending.payment', $sale->id) }}">Edit</a></td>
                            </tr>
                            @endforeach
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form-add-pending-payment" method="POST" action="{{ route('post.sale.pending.payment') }}">
                    {{ csrf_field() }}
                    <input type="hidden" class="form-control" name="invoice_id" id="invoice_id" value="{{ $invoice_id }}" />
                    ...
                    {{-- <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="amount_to_pay" value="{{ $total_remaining }}" placedholder="Amount To Be Paid" readonly required />
                    </div> --}}
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="tds_income_tax_amount" name="tds_income_tax" placeholder="TDS Income Tax Amount">
                    </div>
                    @else
                    ...
                    {{-- <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="amount_remaining" name="amount_remaining" placeholder="Amount Remaining" readonly required/>
                    </div> --}}
                    ...
                    <div class="form-group {{ $showErrors ? ' has-error' : '' }}">
                        ...
                        <input type="text" id="voucher_no" class="form-control" name="voucher_no" placeholder="Voucher No." @if ( $myerrors->has('voucher_no') ) required @else @if($errors->has('voucher_no')) {{-- readonly value="{{ $invoice_no + 1 }}" --}} @else @if(isset(auth()->user()->receiptSetting) && auth()->user()->receiptSetting->bill_no_type == 'auto') value="{{ $voucher_no + 1 }}" readonly @else required @endif @endif @endif />
...
@section('scripts')
    <script>
        ...
        function roundToTwo(num) {
            ...
            @if(auth()->user()->roundOffSetting->round_off == 'yes')

                @if(auth()->user()->roundOffSetting->round_off_to == 'normal')
                    return +(Math.round(num + "e+2")  + "e-2");
                @elseif(auth()->user()->roundOffSetting->round_off_to == 'up')
                    return +(Math.ceil(num + "e+2")  + "e-2");
                @elseif(auth()->user()->roundOffSetting->round_off_to == 'down')
        ...
        function calculate_amount_remaining()
        {
            ...
            var amount_remaining = roundToTwo(amount_to_pay - amount_paid);
        ...
        $(document).ready(function(){
            ...
            $("#payment_date").on("keyup", function() {
                var date = $(this).val();
                var validate_against = '{{ \Carbon\Carbon::parse($invoice_date)->format("d/m/Y") }}';
            ...
            function validateBillNo(party = undefined, bill_no = undefined, userId = undefined) {
                ...
                if(party && bill_no && userId){
                    $.ajax({
                        type: 'post',
                        url: "{{ route('api.validate.sale.payment.voucherno') }}",
                        data: {
                            "token": bill_no,
                            "party": party,
                            "user": userId
            ...
            //         $.ajax({
            //             type: 'post',
            //             url: "{{ route('post.sale.pending.payment') }}",
            //             data: {
            //                 "party_id": party_id,
