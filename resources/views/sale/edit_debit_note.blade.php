...
{{-- {!! Breadcrumbs::render('invoice-detail-debit-note', request()->segment(2)) !!} --}}
    ...
    <form method="post" action="{{ route('update.sale.debit.note') }}">
        {{ csrf_field() }}
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>@if(auth()->user()->profile->inventory_type != "without_inventory") Revised Qty @endif</th>
                    ...
                    <tbody>
                        ...
                        @if(count($debit_notes) > 0)
                            ...
                            <tr>
                                ...
                                <td>{{ $debit_note->item_name }}<input type="hidden" name="item_id[]" value="{{ $debit_note->item_id ?? 0 }}" disabled /></td>
                                
                                <td><input data-gst="{{ auth()->user()->profile->registered == 0 || auth()->user()->profile->registered == 3 ? 0 : $debit_note->item_gst ?? 0 }}" type="text" class="form-control" name="price_difference[]" value="{{ $debit_note->price ?? 0 }}" disabled /></td>
                                ...
                                <td><input data-gst="{{ auth()->user()->profile->registered == 0 || auth()->user()->profile->registered == 3 ? 0 : $debit_note->item_gst ?? 0 }}" type="text" class="form-control" name="quantity_difference[]" value="{{ $debit_note->original_qty ?? 0 }}" @if(auth()->user()->profile->inventory_type == "without_inventory") style="visibility:hidden" @endif disabled /></td>
                                <td>
    ...
    <div class="modal" id="edit-note-modal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form-note" method="post" action="{{ route('update.sale.debit.note.item') }}">
                        {{ csrf_field() }}
        ...
        $(document).ready(function(){
            ...
            $('#note_reason_change').on("change", function() {
                ...
                if( $(this).val() == "discount_on_sale" ){
                    // localStorage.setItem("gst", $('input[name="modal_revised_gst"]').val());
                    $('input[name="modal_revised_gst"]').val(0).attr("disabled", true).trigger('keyup');
                } else {
                    if( localStorage.getItem("gst") !== null ){
                        $('input[name="modal_revised_gst"]').val( localStorage.getItem("gst") ).attr("disabled", true);
